...
let PopoverFiltersComponent = class PopoverFiltersComponent {
    constructor(_httpProvider) {
    ...
    ionViewWillEnter() {
        this.subscribes.push(this.genders$.subscribe(genders => this.genders = genders.genres));
    }
    doFilter() {
...
PopoverFiltersComponent.ctorParameters = () => [
...
let HomePage = class HomePage {
    constructor(_httpProvider, _navCtrl, _popoverController) {
        this._httpProvider = _httpProvider;
    ...
    ionViewWillEnter() {
        this._currentSubcribe = this.releasesMovies$.subscribe(releasesMovies => this._fillFields(releasesMovies));
    }
    ionViewWillLeave() {
    ...
    _segmentChanged(event) {
        ...
        if (this._segmentCurrent === "releases") {
            ...
            this._currentSubcribe = this.popularMovies$.subscribe(popularMovies => this._fillFields(popularMovies));
        }
        else {
            this._currentSubcribe = this.topRatedMovies$.subscribe(topRatedMovies => this._fillFields(topRatedMovies));
        }
    }
    _fillDefaultStateObservables() {
    ...
    _fillFields(responseAPI, incrementResults) {
        ...
        this.movies = incrementResults ?
            [...this.movies, ...responseAPI.results] : responseAPI.results;
    }
    loadData(event) {
        ...
        if (this._segmentCurrent === "releases") {
            ...
            this._currentSubcribe = this.releasesMovies$.subscribe(releasesMovies => {
                this._fillFields(releasesMovies, true);
                event.target.complete();
            });
        }
        else if (this._segmentCurrent === "popular") {
            this.popularMovies$ = this._httpProvider.getPopularMovies(nextPage);
            this._currentSubcribe = this.popularMovies$.subscribe(popularMovies => {
                this._fillFields(popularMovies, true);
                event.target.complete();
            });
        }
        else {
            this.topRatedMovies$ = this._httpProvider.getTopRatedMovies(nextPage);
            this._currentSubcribe = this.topRatedMovies$.subscribe(topRatedMovies => {
                this._fillFields(topRatedMovies, true);
                event.target.complete();
            });
        }
    }
    _disableInfinityScroll() {
